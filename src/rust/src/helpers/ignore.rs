...
pub fn add_gitignore_entry(path: &PathBuf) -> Result<()> {
    ...
    // get relative path
    let ignore_entry_temp = repo::get_relative_path(&dir, path).with_context(|| format!("could not get relative path for {}", path.display()))?;
    // Add leading slash
    ...
    if !ignore_file.exists() {
       File::create(&ignore_file).with_context(|| format!("could not write to gitignore file: {}", ignore_file.display()))?;
    }
