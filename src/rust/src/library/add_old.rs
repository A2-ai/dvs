...
pub fn add(globs: &Vec<String>, message: &String, strict: bool) -> Result<Vec<AddedFile>> {
    // Get git root
    ...
    for file in queued_paths { // had to use for loop instead of map because add returns a result
        match add_file(&file, &git_dir, &conf, &message, strict) {
            Ok(file) => {
...
fn add_file(local_path: &PathBuf, git_dir: &PathBuf, conf: &config::Config, message: &String, strict: bool) -> Result<AddedFile> {
