...
Optionally, it also sets the permissions and primary group of all files copied to the storage directory. These optional arguments don't affect files that are already present  
...
-   the first argument is the relative or absolute path to the desired storage directory. If this directory doesn't exist, devious creates it.
...
-   the storage directory must be a path to a directory, not a file. Devious doesn't assume the user meant to indicate the directory of the file to prevent accidental initialization.
...
-   To keep track of initialization information, Devious generates a `dvs.yaml` file in the project directory. When performing subsequent commands, Devious refers to the information in this file. To change this information, users can run the initialization command again or edit the file directly. (TODO: insert picture here)
