...
Get the file hash
```{r}
assembly_hash <- added_files$hash
original_hash
```
...
add_dir <- dvs_add("data")
add_dir
```
Some other error examples include not being able to get the size or hash for a file. The reason for an error is given in the `error_type` column. If an error message was returned, this will be in the `error_message` column.
