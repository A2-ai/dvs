...
data <- data.frame(
  ...
  Bugs = c("Ladybug", "Firefly", "Bumblebee")
)
...
```{r}
# now we'll add this data to dvs so it can be retrieved by others later
dvs_add(nature_csv_path)
...
# For any other error, dvs will list it in the output
...
added_files <- dvs_add(c(nature_csv_path, "data"), one_df = FALSE)
```
...
data <- data.frame(
  ...
  Bugs = c("Butterfly", "Grasshopper", "Dragonfly")
)
...
```{r}
# I can update my file in dvs by just re-adding it
