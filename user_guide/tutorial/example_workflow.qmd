...
my_storage_dir <- "/data/dvs/jenna"
init <- dvs_init(my_storage_dir, group = "datascience", permissions = 777)
init
```
...
For example, if devious is unable to write to the `.gitignore`, the `dvs_add` will fail to copy the file to the storage directory.
...
I'll run a status check with `dvs_status` in the console to see which files have been added to devious.
...
-   unsynced
    ...
    -   unsynced files may be ahead of or behind current work

-   error

    -   if there's an error getting the status of a file, for example if the path is actually a directory.
